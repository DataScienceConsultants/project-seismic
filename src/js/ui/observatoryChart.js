...
export function renderObservatoryChartsLoading() {
  const { status } = getElements();
  if (status) status.textContent = "Loading historical activity…";
}
...
export function renderObservatoryCharts(data, days = 30, rangeLabel = "Last 30 days") {
  ...
  const observedScores = anomalyScores.filter(value => value !== null).length;
  const observedCounts = eventCounts.filter(value => value !== null).length;
  ...
  elements.anomalySummary.textContent = `${observedScores} daily anomaly score observations shown; gaps indicate unavailable values.`;
  elements.eventSummary.textContent = `${observedCounts} daily event count observations shown; gaps indicate unavailable values.`;
  elements.status.textContent = "Historical activity loaded.";
}

export function renderObservatoryChartsError() {
  const elements = getElements();
  destroyCharts();
